  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "32px", fontFamily: "var(--font-sans)" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          <div style={{ color: "#F25C05", fontSize: "0.75rem", fontWeight: "700", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "8px" }}>Platform Intelligence</div>
      ...
      {/* Recent Activity */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "20px" }}>
        <ActivityCard title="Latest Leads" emoji="👤">
        ...
        <ActivityCard title="New Listings" emoji="🏠">
        ...
        <ActivityCard title="Recent Bookings" emoji="📅">
...
function StatCard({ label, value, caption, emoji, accent }: { label: string; value: string; caption: string; emoji: string; accent?: boolean }) {
  return (
    <div style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: "12px", padding: "24px", position: "relative", overflow: "hidden", boxShadow: "0 1px 3px rgba(0,0,0,0.02)" }}>
      <h3 style={{ fontSize: "0.8rem", color: "#64748B", textTransform: "uppercase", fontWeight: "700", letterSpacing: "0.05em", marginBottom: "16px", margin: "0 0 16px 0" }}>{label}</h3>
      ...
      <div style={{ position: "absolute", bottom: "-20px", right: "-10px", opacity: 0.08, fontSize: "6rem", lineHeight: "1", pointerEvents: "none", zIndex: 0 }}>{emoji}</div>
    </div>
  );
}

function ActivityCard({ title, emoji, children }: { title: string; emoji: string; children: React.ReactNode }) {
  return (
